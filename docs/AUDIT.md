...
| 检查项 | 状态 | 关键指标 | 阈值 |
| --- | --- | --- | --- |
| 1. Token n-gram 重叠（n=8） | ✅ 通过 | 整体 containment 0.4568%（Jaccard 0.2488%）；单文件最高 13.8298%；最长公共 token 连续片段 0 | 整体 containment ≤ 2.00%；且**任一文件**不得存在 ≥ 25 token 的连续公共片段（该判据不受文件大小豁免）；单文件 containment > 5.00% 且无长连续片段者列为待复核 |
...
| 5. 资产 / 文档审计（文档 n=12） | ✅ 通过 | 哈希相同资产 0 个；文档整体 containment 0.0342%（最高 0.6594%） | 哈希相同 = 0 且 文档整体 ≤ 2.00% 且 单文档 ≤ 5.00% |

## 1. Token n-gram 重叠（n=8）

**状态：** ✅ 通过　　**关键指标：** 整体 containment 0.4568%（Jaccard 0.2488%）；单文件最高 13.8298%；最长公共 token 连续片段 0　　**阈值：** 整体 containment ≤ 2.00%；且**任一文件**不得存在 ≥ 25 token 的连续公共片段（该判据不受文件大小豁免）；单文件 containment > 5.00% 且无长连续片段者列为待复核

- 上游参考 n-gram 池：**82,410** 个（来自 58 个 `.py` 文件）
- 目标 n-gram 池：**98,080** 个（扫描 96 个 `.py` 文件）
- 交集：**448** 个
...
- 目标字符串字面量（≥20 字符，已跳过 docstring）：**765** 条
...
**状态：** ✅ 通过　　**关键指标：** 哈希相同资产 0 个；文档整体 containment 0.0342%（最高 0.6594%）　　**阈值：** 哈希相同 = 0 且 文档整体 ≤ 2.00% 且 单文档 ≤ 5.00%
...
- 目标 `.md`/`.rst` 文件：**26** 个，n-gram 池 **70,170** 个
- 整体 containment **0.0342%**，Jaccard **0.0015%**
...
| 10 | `docs/eval_agentic_9q_B1.md` | 0.4353% | 4 | 919 |
| 11 | `docs/eval_agentic_9q_crossencoder.md` | 0.4338% | 4 | 922 |
| 12 | `docs/eval_agentic_9q.md` | 0.4320% | 4 | 926 |
| 13 | `docs/eval_agentic_9q_dedup.md` | 0.4306% | 4 | 929 |
| 14 | `docs/eval_agentic_9q_nocap.md` | 0.4306% | 4 | 929 |
| 15 | `docs/eval_agentic_9q_budgetC.md` | 0.4292% | 4 | 932 |
| 16 | `benchmarks/data/eval36_report.md` | 0.4283% | 4 | 934 |
| 17 | `docs/eval_36q_deterministic.md` | 0.4278% | 4 | 935 |
| 18 | `docs/eval_agentic_9q_capA.md` | 0.4278% | 4 | 935 |
| 19 | `docs/eval_agentic_9q_Cprime.md` | 0.4260% | 4 | 939 |
| 20 | `tools/README.md` | 0.2564% | 2 | 780 |
