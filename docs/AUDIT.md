...
| 检查项 | 状态 | 关键指标 | 阈值 |
| --- | --- | --- | --- |
| 1. Token n-gram 重叠（n=8） | ✅ 通过 | 整体 containment 0.4592%（Jaccard 0.2496%）；单文件最高 13.8298%；最长公共 token 连续片段 0 | 整体 containment ≤ 2.00%；且**任一文件**不得存在 ≥ 25 token 的连续公共片段（该判据不受文件大小豁免）；单文件 containment > 5.00% 且无长连续片段者列为待复核 |
| 2. 逐行精确复制 | ✅ 通过 | 逐字相同行 387（实质性 0 / 通用惯用式 387）；最长对齐片段 2 行 | 实质性 ≤ 0 行 且 对齐片段 < 5 行 |
...
| 5. 资产 / 文档审计（文档 n=12） | ✅ 通过 | 哈希相同资产 0 个；文档整体 containment 0.0355%（最高 0.6594%） | 哈希相同 = 0 且 文档整体 ≤ 2.00% 且 单文档 ≤ 5.00% |

## 1. Token n-gram 重叠（n=8）

**状态：** ✅ 通过　　**关键指标：** 整体 containment 0.4592%（Jaccard 0.2496%）；单文件最高 13.8298%；最长公共 token 连续片段 0　　**阈值：** 整体 containment ≤ 2.00%；且**任一文件**不得存在 ≥ 25 token 的连续公共片段（该判据不受文件大小豁免）；单文件 containment > 5.00% 且无长连续片段者列为待复核

- 上游参考 n-gram 池：**82,410** 个（来自 58 个 `.py` 文件）
- 目标 n-gram 池：**97,551** 个（扫描 96 个 `.py` 文件）
- 交集：**448** 个
...
| 8 | `src/scitrace/config/settings.py` | 4.0637% | 97 | 2,387 | 0 |
...
**状态：** ✅ 通过　　**关键指标：** 逐字相同行 387（实质性 0 / 通用惯用式 387）；最长对齐片段 2 行　　**阈值：** 实质性 ≤ 0 行 且 对齐片段 < 5 行

- 上游可比对代码行索引：**12,237** 条唯一非空行（`.py`，已剔除 <12 字符与通用样板）
- 目标侧扫描的非空行：**17,985** 行
- 命中（逐字相同）：**387** 行，其中 **实质性 0** 行、通用惯用式 387 行
...
| --- | --- | --- | --- | --- | --- |
| 1 | 2 | `src/scitrace/config/settings.py` | 247–248 | `src/paperqa/settings.py` | 132–133 |
| 2 | 2 | `src/scitrace/config/settings.py` | 273–274 | `src/paperqa/settings.py` | 132–133 |
...
- 目标字符串字面量（≥20 字符，已跳过 docstring）：**763** 条
...
**状态：** ✅ 通过　　**关键指标：** 哈希相同资产 0 个；文档整体 containment 0.0355%（最高 0.6594%）　　**阈值：** 哈希相同 = 0 且 文档整体 ≤ 2.00% 且 单文档 ≤ 5.00%
...
- 目标 `.md`/`.rst` 文件：**24** 个，n-gram 池 **67,694** 个
- 整体 containment **0.0355%**，Jaccard **0.0015%**
...
| 13 | `docs/eval_agentic_9q_nocap.md` | 0.4306% | 4 | 929 |
| 14 | `benchmarks/data/eval36_report.md` | 0.4283% | 4 | 934 |
| 15 | `docs/eval_36q_deterministic.md` | 0.4278% | 4 | 935 |
| 16 | `docs/eval_agentic_9q_capA.md` | 0.4278% | 4 | 935 |
| 17 | `docs/eval_agentic_9q_Cprime.md` | 0.4260% | 4 | 939 |
| 18 | `tools/README.md` | 0.2564% | 2 | 780 |
| 19 | `docs/AGENTIC_PLAN.md` | 0.1079% | 5 | 4,636 |
| 20 | `docs/SPEC.md` | 0.0957% | 9 | 9,403 |
